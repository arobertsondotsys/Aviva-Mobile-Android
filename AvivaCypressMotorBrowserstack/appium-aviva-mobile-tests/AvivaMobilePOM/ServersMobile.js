...
class Servers {
    ...
    constructor() {
        // Default server
        this.server = process.env.SERVER || 'QA2'
        ...
        this.server1Urls = {
            ...
            TEST3FAT: 'https://insurance-testaviva3fat.dotsys.co.uk/myaviva/login.aspx?',
    ...
    // ---- Load Back Office ----
    async openBackOffice(driver) {
        const url = this.serverUrls[this.server] || this.serverUrls['QA2']
        await driver.url(url)
    }

    async openMyAviva(driver) {
        const url = this.server1Urls[this.server] || this.server1Urls['QA2']
        await driver.url(url)
    }

    async openDemoLinks(driver) {
        const url = this.server2Urls[this.server] || this.server2Urls['QA2']
        await driver.url(url)
    }
}
